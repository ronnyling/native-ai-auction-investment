...
FROM "Properties"
WHERE days_to_auction <= 7 AND !contains(list("passed","pass"), status)
...
FROM "Properties"
WHERE auction_count > 1 AND !contains(list("passed","pass"), status)
...
FROM "Properties"
WHERE contains(list("reviewing","interested","shortlisted","visiting","bid"), status)
SORT auction_date ASC
```
